...
/*
<applet code="SDSApplet" width=600 height=600 />
*/
...
public class SDSApplet extends Applet implements Runnable {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private int nSize = 50;
	private static int nZc = 2;
	private int[][] pile;
		...
		while (bCritical) {
			...
			for (int i=1; i<=nSize; i++) {
				for (int j=1; j<=nSize; j++) {
					if (pile[i][j] >= nZc) {
						pile[i][j] -= nZc;
						j1 = (int)(3.0*Math.abs(Math.random()))-1 + j;
						j2 = (int)(3.0*Math.abs(Math.random()))-1 + j;
						pile[i+1][j1]++;
						pile[i+1][j2]++;
						if (!changed[i][j]) {
							nAvalSize++;
